...
export class VolunteersService {
  ...
  updateVolUnAvailableDate(id: string, date: string) {
    console.log(id, date);
    this.volunteerDoc = this.afs.doc(`mpt_volunteer/${id}`);
    this.volunteerDoc.update({
      dateUnAvailable: firestore.FieldValue.arrayUnion(date),
    });
  }
  ...
  deleteVolUnAvailableDate(date: string, vol: string) {
    ...
    this.volunteerDoc.update({
      dateUnAvailable: firestore.FieldValue.arrayRemove(date),
    });
  }
}
